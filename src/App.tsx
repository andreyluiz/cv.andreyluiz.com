    ...
    <div className="max-w-4xl mx-auto p-12 bg-white print:text-sm">
      ...
      <header className="mb-8 print:mb-4">
        <div className="flex items-baseline">
          <h1 className="text-4xl font-bold text-gray-800 print:text-2xl">
            {personalInfo.name}
          </h1>
      ...
      <section className="mb-8 print:mb-4">
        <h2 className="text-2xl font-semibold text-gray-800 border-b-2 border-gray-300 pb-2 mb-3 print:mb-2 print:text-lg">
          Professional Summary
        </h2>
        <p className="text-gray-700 text-justify print:text-sm">{summary}</p>
      </section>
      ...
      <section className="mb-8 print:mb-4">
        ...
        <div className="grid grid-cols-2 gap-6 print:gap-4">
          <div className="bg-gray-50 p-4 print:p-3 rounded-lg">
            <h3 className="font-semibold mb-3 print:mb-2 text-gray-800 print:text-base">
              Programming Languages
            </h3>
            <ul className="list-disc list-inside text-gray-700 space-y-1 print:space-y-0.5 print:text-xs">
          ...
          {recentExperiences.map((exp, index) => (
            <div key={index} className="bg-gray-50 rounded-lg">
              ...
              {exp.achievements.length > 0 && (
                <ul className="list-disc list-inside text-gray-700 p-3 print:p-2 space-y-1 print:space-y-0.5 print:text-xs">
      ...
      <section className="mb-8 print:mb-4 print:pt-14 print:break-before-all">
        ...
        <div className="grid grid-cols-1 gap-2 print:gap-1 print:text-sm">
          {previousExperiences.map((exp, index) => (
            <div
              key={index}
              className="bg-gray-50 p-3 print:p-2 rounded-lg flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 print:gap-1"
            >
              <div>
      ...
      <section className="mb-8 print:mb-4">
        ...
        <div className="grid grid-cols-1 gap-4 print:gap-2 print:text-sm">
          {projects.map((project, index) => (
            <div key={index} className="bg-gray-50 p-4 print:p-3 rounded-lg">
              <h3 className="text-lg font-semibold text-gray-800 mb-2 print:text-base">
                {project.name}
              </h3>
              <p className="text-gray-700 print:text-xs">{project.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-8 print:mb-4">
        ...
        <div className="space-y-4 print:space-y-2 print:text-sm">
          {education.map((edu, index) => (
            <div key={index} className="bg-gray-50 p-3 print:p-2 rounded-lg">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline gap-1">
                <h3 className="text-xl font-semibold text-gray-800 print:text-base">
                  {edu.degree}
                </h3>
                <span className="text-gray-600">{edu.year}</span>
              </div>
              <p className="text-gray-700 mt-1 print:text-xs">{edu.institution} - {edu.location}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          {publications.map((pub, index) => (
            <div key={index} className="bg-gray-50 p-3 print:p-2 rounded-lg">
              <a
                ...
                className="text-blue-600 hover:text-blue-800 hover:underline print:text-gray-600"
              >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 print:gap-2 print:text-sm">
          {languages.map((language, index) => (
            <div key={index} className="bg-gray-50 p-3 print:p-2 rounded-lg">
              ...
              <p className="text-gray-600 text-sm print:text-xs">{language.level}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Print Source */}
      <div className="hidden print:block mt-6 pt-3 text-center text-gray-400 text-xs border-t border-gray-200">
